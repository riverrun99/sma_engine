...
```bash
git clone https://github.com/YOUR_USERNAME/sma_engine
cd sma_engine
cp .env.example .env
...
Add to `muted_tickers.txt`, one per line:

```
VTIP    # bond ETF, noise hits
ESPR    # acted on, removing
```

Or batch-mute from the command line:

```bash
python3 mute.py ESPR CCRN VTIP
```

`muted_tickers.txt` is hot-reloaded every cycle — no restart needed.

### Unmuting

```bash
python3 mute.py --unmute ESPR
```

### Adding custom tickers

Open `custom_tickers.txt` and add tickers one per line. These are hot-reloaded
every cycle and stack on top of the configured universe tier.
